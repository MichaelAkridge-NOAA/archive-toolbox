...
def process_blob(blob, temp_folder):
    try:
        ...
        logging.error(f"An error occurred while processing {blob.name}: {e}")
...
def clean_xml_files_in_bucket(bucket_name, prefix, temp_folder):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(process_blob, blob, temp_folder) for blob in blobs if blob.name.endswith(".xml")]
        
        for future in as_completed(futures):
            try:
                future.result()
            except Exception as e:
                logging.error(f"An error occurred during concurrent processing: {e}")
