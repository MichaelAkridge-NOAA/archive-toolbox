...
def gather_metadata(bucket_name, prefix, db_path, batch_size=100, retry_limit=3):
    ...
    logging.info("Fetching blobs for prefix: %s", prefix)
    count = 0
    for blob in blobs:
        count += 1
    ...
    logging.info("Completed fetching %d blobs for prefix: %s", count, prefix)
    ...
    for subfolder in blobs.prefixes:
        ...
        while retries < retry_limit:
            try:
                ...
                retries += 1
                logging.error("Error gathering metadata for %s: %s. Retrying (%d/%d)...", subfolder, e, retries, retry_limit)
                time.sleep(5)
        if retries == retry_limit:
            logging.error("Failed to gather metadata for %s after %d attempts.", subfolder, retry_limit)
...
def main():
    ...
    if last_prefix:
        logging.info("Resuming from last processed prefix: %s", last_prefix)
        prefix = last_prefix

    logging.info("Gathering metadata from Google Cloud Storage...")
    try:
        gather_metadata(bucket_name, prefix, db_path)
    except Exception as e:
        logging.error("An error occurred while gathering metadata: %s", e)
        return
    
    logging.info("Metadata gathering completed and stored in database.")
    ...
    logging.info("Statistics saved to %s", csv_path)
